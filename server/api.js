...
router.post("/signup", async (req, res) => {
	try {
		...
		pool
		.query("SELECT username FROM users WHERE username=$1", [newUser.username])
		.then((result) => {
			if (result.rows.length > 0) {
				res.status(400).send("Username already exist!");
			} else {
				//Checking if cohort ID exist
				pool.query("SELECT id FROM cohorts WHERE id=$1", [cohortId])
				.then((result1) => {
					if (result1.rows.length < 1) {
						res.status(400).send("Cohort ID doesn't exist");
					//ensuring that all fields are filled
					} else if (newUser.firstName && newUser.lastName && newUser.username && newUser.cohortId && newUser.password && !newUser.isVolunteer) {
